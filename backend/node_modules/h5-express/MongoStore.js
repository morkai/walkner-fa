...
function MongoStore(db, options)
{
   ...
   * @type {{key: {}}[]}
   */
  this.indexes = [{key: {expires: 1}}].concat(options.indexes || []);
  ...
  this.db.on('reconnected', this.onOpen);
...
MongoStore.prototype.touch = function(sid, session, done)
{
  ...
  sessions.updateOne({_id: sid}, {$set: {expires, 'data.updatedAt': now}}, err =>
  {
    if (done)
    {
      done(err);
...
MongoStore.prototype.destruct = function()
{
  ...
  this.db.removeListener('reconnected', this.onOpen);
 ...
 * @param {function} [done]
 */
MongoStore.prototype.createIndexes = function(done)
{
  this.collection().createIndexes(this.indexes, err =>
  {
    if (done)
    {
      done(err);
