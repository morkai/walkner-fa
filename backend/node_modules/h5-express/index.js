...
try { checkDiskSpace = require('check-disk-space').default; } catch (err) {}
...
exports.start = (app, expressModule) =>
{
  ...
  if (!checkDiskSpace) expressModule.debug('Unused dependency: check-disk-space');
  if (!messageFormatAmd) expressModule.debug('Unused dependency: messageformat-amd');
  if (!cookieParser) expressModule.debug('Unused dependency: cookie-parser');
  if (!bodyParser) expressModule.debug('Unused dependency: body-parser');
  if (!iconv) expressModule.debug('Unused dependency: iconv-lite');
  if (!session) expressModule.debug('Unused dependency: express-session');
  if (!pmx) expressModule.debug('Unused dependency: pmx');
  if (!MongoStore) expressModule.debug('Unused dependency: MongoStore');
  /* eslint-enable no-empty */
  ...
  if (checkDiskSpace && config.checkDiskSpace && config.checkDiskSpace.minFreeSpace > 0)
  {
  ...
  if (expressModule.sessionStore)
  {
    ...
    expressApp.use((req, res, next) =>
    {
      ...
      else
      {
        ...
        app.broker.publish('express.fakeSessionCreated', {req});
