...
exports.start = (app, module, done) =>
{
  ...
  _.forEach(module.config.broadcastTopics, (broadcastTopic) =>
  {
    app.broker.subscribe(broadcastTopic, (message, topic, meta) =>
    {
      ...
      module.request('@broadcast', {topic, message, meta});
    });
  });
  ...
  function subscribe()
  {
    if (_.isEmpty(module.config.subscribeTopics))
    {
      return;
    }

    module.request('@subscribe', {topics: module.config.subscribeTopics}, (err) =>
    {
      if (err)
      {
        module.error(err, 'Failed to subscribe.');
      }
    });
  }
};

function monkeyPatch(app)
{
  axon.ReqSocket.prototype.onmessage = function()
  {
    const self = this;
    ...
    return function(buf)
    {
      ...
      if (!fn)
      {
        if (id === '@publish' && args.length && args[0].topic)
        {
          app.broker.publish(
            args[0].topic,
            args[0].message,
