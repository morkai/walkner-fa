// Part of <https://miracle.systems/p/walkner-fa> licensed under <CC BY-NC-SA 4.0>

'use strict';

const step = require('h5.step');
const locks = require('util/locks');

module.exports = (app, module, req, res, next) =>
{
  const {
    user,
    FaOt,
    FaReqTpl
  } = module;

  step(
    function()
    {
      this.unlockEntry = locks.create(`${FaOt.TOPIC_PREFIX}.${req.params.id}`, this.next());
    },
    function()
    {
      FaOt.findById(req.params.id).exec(this.next());
    },
    function(err, doc)
    {
      if (err)
      {
        return this.skip(app.createError(
          `Failed to find a document: ${err.message}`,
          'FIND_FAILURE',
          500,
          {documentId: req.params.id}
        ));
      }

      if (!doc)
      {
        return this.skip(app.createError('Document not found.', 'NOT_FOUND', 404));
      }

      delete req.body.protocolNo;
      delete req.body.documentNo;

      const sessionUser = req.session.user;
      const props = Object.keys(req.body);
      const canEdit = doc.canEdit(sessionUser);
      const canComment = user.isAllowedTo(sessionUser, [['FA:VIEW'], ['FA:OT:VIEW']]);
      const commentOnly = props.includes('comment') && props.length === 1;

      if (!canEdit)
      {
        if (!canComment || !commentOnly)
        {
          return this.skip(app.createError('Not allowed.', 'NO_AUTH', 403));
        }
      }

      claimAttachments(props);

      this.doc = doc;

      module.generateNextNo(doc, req.body.newStage, this.next());
    },
    function(err, unlockNoGenerator)
    {
      if (err)
      {
        return this.skip(app.createError(
          `Failed to generate the next document number: ${err.message}`,
          'NO_GENERATION_FAILURE',
          500
        ));
      }

      this.unlockNoGenerator = unlockNoGenerator;

      this.changed = this.doc.applyChanges(req.body, user.createUserInfo(req.session.user, req));

      if (!this.changed)
      {
        return;
      }

      const change = this.doc.changes[this.doc.changes.length - 1];
      const oldStage = change.data.stage && change.data.stage[0] || null;
      const newStage = change.data.stage && change.data.stage[1] || null;

      if (!this.doc.hrtFile && newStage === 'accept')
      {
        FaReqTpl.attach(this.doc, change, this.next());
      }
      else if (this.doc.hrtFile
        && oldStage === 'accept'
        && FaOt.STAGES.indexOf(newStage) < FaOt.STAGES.indexOf('accept'))
      {
        change.data.hrtFile = [this.doc.hrtFile.toObject(), null];
        this.doc.hrtFile = null;
      }
    },
    function(err)
    {
      if (err)
      {
        return this.skip(app.createError(
          `Failed to generate the HRT file: ${err.message}`,
          'SAVE_FAILURE',
          500,
          {documentId: req.params.id}
        ));
      }

      if (this.changed)
      {
        this.doc.save(this.next());
      }
    },
    function(err)
    {
      if (err)
      {
        return this.skip(app.createError(
          `Failed to save a document: ${err.message}`,
          'SAVE_FAILURE',
          500,
          {documentId: req.params.id}
        ));
      }
    },
    function(err)
    {
      setImmediate(this.unlockEntry);

      if (this.unlockNoGenerator)
      {
        this.unlockNoGenerator();
      }

      if (err)
      {
        return next(err);
      }

      if (!this.changed)
      {
        return res.sendStatus(204);
      }

      res.json(this.doc);

      app.broker.publish(`${FaOt.TOPIC_PREFIX}.edited`, {
        model: this.doc,
        user: this.doc.updatedBy
      });

      const change = this.doc.changes[this.doc.changes.length - 1].toJSON();
      const omit = ['changes'].concat(Object.keys(change.data));

      app.broker.publish(`${FaOt.TOPIC_PREFIX}.updated.${this.doc._id}`, {
        _id: this.doc._id,
        socketId: req.headers['x-wmes-socket'] || null,
        values: this.doc.lastModifiedValues({omit}),
        change
      });
    }
  );

  function claimAttachments(props)
  {
    props.forEach(prop =>
    {
      const data = req.body[prop];

      if (prop === 'assets' && Array.isArray(data))
      {
        data.forEach(asset =>
        {
          Object.keys(asset).forEach(subProp =>
          {
            if (subProp.endsWith('File'))
            {
              claimAttachment(asset[subProp]);
            }
          });
        });
      }
      else if (prop.endsWith('File'))
      {
        claimAttachment(data);
      }
    });
  }

  function claimAttachment(attachment)
  {
    if (!attachment)
    {
      return;
    }

    const tmpAttachment = module.tmpAttachments.get(attachment.hash);

    if (!tmpAttachment)
    {
      return;
    }

    clearTimeout(tmpAttachment.timer);

    module.tmpAttachments.delete(attachment.hash);
  }
};
