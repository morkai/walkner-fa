  ...
  function serializeRow(doc, req)
  {
    const zplx = doc.zplx
      .map(zplx =>
      {
        ...
        return `${zplx.code} (${value})`;
      })
      .join('\r\n');
    const committee = doc.committee.map(u => u.label).join('\r\n');
    const comments = doc.changes
      .filter(c => !!c.comment)
      .map(c => `${c.user.label} @ ${moment(c.date).format('L LT')}: ${c.comment.replace(/\n/g, '\t\n')}`)
      .join('\r\n---\r\n');

    return doc.assets.map((asset, i) =>
    {
      const assetClass = req.dictionaries.assetClasses[asset.assetClass];
      ...
      return {
        ...
        lineSymbol: asset.lineSymbol,
        zplx,
        owner: asset.owner ? asset.owner.label : '',
        committee,
        supplier: asset.supplier,
        costCenter: asset.costCenter,
        evalGroup1: asset.evalGroup1,
        evalGroup5: asset.evalGroup5,
        assetClass: assetClass ? assetClass.name : asset.assetClass,
        depRate: asset.depRate / 100,
        depKey: asset.depKey,
        economicMethod: asset.economicMethod,
        fiscalMethod: asset.fiscalMethod,
        taxMethod: asset.taxMethod,
        economicPeriod: asset.economicPeriod,
        fiscalPeriod: asset.fiscalPeriod,
        taxPeriod: asset.taxPeriod,
        economicDate: asset.economicDate,
        fiscalDate: asset.fiscalDate,
        taxDate: asset.taxDate,
        value: asset.value,
        transactionType: asset.transactions.map(t => t.type).join('; '),
        economicValue: asset.transactions.map(t => t.amount1.toFixed(2).replace('.', ',')).join('; '),
        fiscalValue: asset.transactions.map(t => t.amount2.toFixed(2).replace('.', ',')).join('; '),
        postingDate: doc.postingDate,
        vendorNo: asset.vendorNo,
        vendorName: asset.vendorName,
        assetNo: asset.assetNo,
        accountingNo: asset.accountingNo,
        odwNo: asset.odwNo,
        tplNotes: asset.tplNotes,
        comments: i === 0 ? comments : ''
      };
    });
  }
