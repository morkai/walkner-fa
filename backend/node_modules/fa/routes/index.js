...
const sendFileRoute = require('./sendFile');
const dictionariesRoute = require('./dictionaries');
const reqTplsRoutes = require('./reqTpls');
...
module.exports = (app, module) =>
{
  ...
  const canView = user.auth('FA:VIEW', 'FA:OT:VIEW', 'FA:LT:VIEW');
  const canManage = user.auth('FA:MANAGE');
  const canAttach = canView;
  ...
  express.post(
    ...
    multer({
      ...
      limits: {
        fileSize: module.config.maxAttachmentSize
      }
    }).single('file'),
  ...
  express.post(
    '/fa/files/:id',
    user.auth('FA:MANAGE', 'FA:MANAGE:FILES'),
    multer({
      dest: module.config.uploadsDest,
      limits: {
        fileSize: module.config.maxFileSize
      }
    }).single('file'),
    uploadFileRoute.bind(null, app, module)
  );
  express.get(
    '/fa/files/:id',
    canView,
    sendFileRoute.bind(null, app, module)
  );

  reqTplsRoutes(app, module);
  otRoutes(app, module);
  ltRoutes(app, module);
};
