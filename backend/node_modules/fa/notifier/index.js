...
module.exports = (app, module) =>
{
  ...
  {
    notifyUsers({_id: new ObjectId(doc.stageChangedBy.document._id)}, 'ot.verify.rejected', doc);
  }
  ...
  {
    notifyUsers({_id: new ObjectId(doc.stageChangedBy.verify._id)}, 'ot.accept.rejected', doc);
  }

  function handleOtAccept(doc)
  {
    notifyUsers({_id: new ObjectId(doc.owner._id)}, 'ot.accept', doc);
  }
  ...
  {
    notifyUsers({_id: new ObjectId(doc.stageChangedBy.document._id)}, 'ot.finished', doc);
  }
  ...
  function handleLtAcceptCommittee(doc)
  {
    ...
    const committee = doc.committee.map(user => new ObjectId(user._id));

    notifyUsers({_id: {$in: committee}}, 'lt.acceptCommittee', doc);
  }
  ...
  {
    // TODO Who is the LT's owner?
    // notifyUsers({_id: new ObjectId(doc.owner._id)}, 'lt.acceptOwner', doc);
  }

  function handleLtAcceptOwnerRejected(doc)
  {
    const users = [
      new ObjectId(doc.stageChangedBy.protocol._id),
      new ObjectId(doc.stageChangedBy.verify._id)
    ];

    notifyUsers({_id: {$in: users}}, 'lt.acceptOwner.rejected', doc);
  }
  ...
  {
    const users = [
      new ObjectId(doc.stageChangedBy.acceptOwner._id)
    ];

    notifyUsers({_id: {$in: users}}, 'lt.acceptFinance.rejected', doc);
  }
  ...
  function handleLtAcceptFinanceCancelled(doc)
  {
    ...
    notifyUsers({_id: {$in: users}}, 'lt.acceptFinance.cancelled', doc);
  }
  ...
  function handleLtAcceptDepartmentRejected(doc)
  {
    const users = [
      new ObjectId(doc.stageChangedBy.acceptOwner._id),
      new ObjectId(doc.stageChangedBy.acceptFinance._id)
    ];

    notifyUsers({_id: {$in: users}}, 'lt.acceptDepartment.rejected', doc);
  }
  ...
  function handleLtAcceptDepartmentCancelled(doc)
  {
    ...
    notifyUsers({_id: {$in: users}}, 'lt.acceptDepartment.cancelled', doc);
  }
  ...
  function handleLtAcceptDocument(doc)
  {
    ...
    notifyUsers({_id: {$in: users}}, 'lt.acceptDocument', doc);
  }
  ...
  function handleLtAcceptDocumentRejected(doc)
  {
    const users = [
      new ObjectId(doc.stageChangedBy.acceptFinance._id),
      new ObjectId(doc.stageChangedBy.acceptDepartment._id)
    ];

    notifyUsers({_id: {$in: users}}, 'lt.acceptDocument.rejected', doc);
  }
  ...
  {
    notifyUsers({_id: new ObjectId(doc.stageChangedBy.acceptDocument._id)}, 'lt.finished', doc);
  }
