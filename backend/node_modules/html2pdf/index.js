...
exports.start = (app, module) =>
{
  ...
  module.generateBarCode = (options, done) =>
  {
    if (!done)
    {
      ...
      {
        module.generateBarCode(options, (err, qrCode) =>
        {
          if (err)
          {
            return reject(err);
    ...
    step(
      ...
      function(err, stdout, stderr)
      {
        ...
        done(null, options && options.encoding === 'buffer' ? stdout : stdout.toString('base64'));
      }
    );
  };
        ...
        module.puppeteerPool.acquire()
          .then(async browser =>
          {
            try
            {
              ...
              if (!res || !res.ok())
              {
                throw app.createError(`Expected 200, got ${res ? res.status() : 0}`, 'INVALID_STATUS', 500);
              }
  ...
  module.printPdf = (hash, printer, settings, done) =>
  {
    if (done)
    {
      return printPdf(hash, printer, settings, done);
    }

    return new Promise((resolve, reject) =>
    {
      printPdf(hash, printer, settings, err =>
      {
        if (err)
        {
          reject(err);
  ...
  function printPdf(hash, printer, settings, done)
  {
  ...
  module.findPrinter = (options, done) =>
  {
    if (typeof done === 'function')
    {
      findPrinter(options, done);
    }
    else
    {
      return new Promise((resolve, reject) =>
      {
        findPrinter(options, (err, printer) =>
        {
          if (err)
          {
            return reject(err);
  ...
  function findPrinter(options, done)
  {
    step(
      function()
      {
      ...
      function(err, printers)
      {
        ...
        if (err)
        {
          return done(err, {linePrinter, printer});
        }

        if (!printers.length)
        {
          return done(app.createError(`Printer not found for tag.`, 'PRINTER_NOT_FOUND', 400, {
        ...
        do
        {
          ...
          linePrinter = findLinePrinter(parseSpecial(printer.special), options.line, options.station);
        }
        while (!linePrinter && printers.length);

        if (options.line && !linePrinter)
        {
          return done(app.createError(`Printer not found for line.`, 'PRINTER_NOT_FOUND', 400, {
            line: options.line,
            printer: options.printer,
            tag: options.tag
          }));
  ...
  function printZpl(zpl, options, done)
  {
    ...
    step(
      function()
      {
        findPrinter(options, this.parallel());
      ...
      function(err, {printer, linePrinter})
      {
        if (err)
        {
          return this.skip(err);
        ...
        if (linePrinter)
        {
          if (linePrinter.host.toLowerCase() === 'local')
          {
            return this.skip(app.createError(
              `Printing by local printer not supported.`, 'INVALID_PRINTER', 400
            ));
          }
  ...
  function findLinePrinter(special, requiredLine, requiredStation)
  {
    if (!special.LINE)
    {
      return null;
    ...
    requiredLine = requiredLine.toUpperCase();

    let wildcardPrinter = null;

    for (const lineOptions of special.LINE)
    {
      ...
      const lineParts = lineOptions[0].split('/');
      const line = lineParts[0] || '*';
      const station = parseInt(lineParts[1], 10) || '*';
      const printer = {
        host: lineOptions[1].trim(),
        port: parseInt(lineOptions[2], 10) || 9100
      };

      if (line === requiredLine && (station === '*' || station === requiredStation))
      {
        return printer;
      }

      if (!wildcardPrinter && line === '*' && (station === '*' || station === requiredStation))
      {
        wildcardPrinter = printer;
      }
    }
  ...
  function logZpl(zpl)
  {
    fs.stat(join(app.options.rootPath, `../data/zpl`), err =>
    {
      if (err)
      {
        return;
      ...
      fs.writeFile(join(app.options.rootPath, `../data/zpl/${moment().format('YYYY-MM-DD_HH-mm-ss-SSS')}.zpl`), zpl, err =>
      {
        if (err)
        {
          module.error(err, 'Failed to log ZPL.', {zpl});
        }
      });
    });
  }
