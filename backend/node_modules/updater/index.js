...
{
  const dictionaryModelsCache = new Map();
  ...
  module.getAppTemplateData = (appId, req, appData) =>
  {
    ...
    return {
      dev: app.options.env === 'development',
      appCacheManifest,
  ...
  module.extendAppData = (appId, appData) =>
  {
    ...
    _.forEach(manifest.dictionaryModules, (appDataKey, moduleName) =>
    {
      ...
      const cache = dictionaryModelsCache.get(dictionaryModule);

      if (cache && cache.updatedAt === dictionaryModule.updatedAt)
      {
        appData[appDataKey] = cache.value;
      ...
      else if (typeof models[0].toDictionaryObject !== 'function')
      {
        appData[appDataKey] = "JSON.parse("
          + JSON.stringify(JSON.stringify(models))
          + ")";
      }
      else
      {
        appData[appDataKey] = "JSON.parse("
          + JSON.stringify(JSON.stringify(_.invokeMap(models, 'toDictionaryObject')))
          + ")";
      }

      dictionaryModelsCache.set(dictionaryModule, {
        updatedAt: dictionaryModule.updatedAt,
        value: appData[appDataKey]
      });
    });
