...
module.exports = (app, module) =>
{
  ...
  function notify(service)
  {
    ...
    {
      const now = Date.now();
      const diff = now - service.lastNotifyAt;
      ...
      {
        logger.debug('Not calling so soon.', {
          lastNotifyAt: formatDateTime(service.lastNotifyAt),
          diff: diff / 1000
        });
      }
    }
  ...
  function call(text)
  {
    const caller = app[module.config.callerId];

    if (!caller || !module.config.appStartedCallRecipient)
    {
      return;
    }
    ...
    caller.call(callOptions, err =>
    {
      if (err)
      {
        logger.error(err, 'Failed to call.', {recipients: [callOptions.to]});
      }
      else
      {
        logger.info(`Called!`, {recipients: [callOptions.to]});
      }
    });
  }
};
