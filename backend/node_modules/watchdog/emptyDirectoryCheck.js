...
module.exports = (app, module) =>
{
  ...
  function notify(config)
  {
    ...
    if (caller && config.callRecipient)
    {
      const callOptions = {
      ...
      caller.call(callOptions, (err) =>
      {
        if (err)
        {
          logger.error(err, `Failed to call!`, {dirId: config.id, recipients: [callOptions.to]});
        }
        else
        {
          logger.info(`Called!`, {dirId: config.id, recipients: [callOptions.to]});
        }
      });
    }
  }
